...
//	$sort=$_GET["sort"]; 
//	if($sort==1) { $order="order by recvname ASC"; }
//	elseif($sort==2) { $order="order by recvname DESC"; }
//	elseif($sort==3) { $order="order by bokdate ASC"; }
//	elseif($sort==4) { $order="order by bokdate DESC"; }
//	else{ $order=""; }
	if($bok_descityid==0)
	{
		$sql="select * from booking bok join des_cities dc on (bok.bok_descityid=dc.dcty_id) join sender s on (bok.bok_senderid=s.sendid) join recivers r on (bok.bok_reciverid=r.recvid) where bok_status='1'";  //$order	
	} 
	elseif($bok_descityid!=0)
	{
		$sql="select * from booking bok join des_cities dc on (bok.bok_descityid=dc.dcty_id) join sender s on (bok.bok_senderid=s.sendid) join recivers r on (bok.bok_reciverid=r.recvid) where bok_descityid='$bok_descityid' && bok_status='1'"; //$order
	}
?>				
<div class="">
		<div class="clearfix"></div>
								...
								<tr>
									 ...
									 <td class="a-center "> <?php ?></td>
										...
										<?php  
                                                                                ...
                                                                                $sql="select * from booking bok join des_cities dc on (bok.bok_descityid=dc.dcty_id) join sender s on (bok.bok_senderid=s.sendid) join recivers r on (bok.bok_reciverid=r.recvid) where bok_loaddate='$bok_loaddate' AND bok_memo='$bok_memo' AND bok_status='1'";	 
										...
										?> 
											<tr class="even pointer">  
												...
												<td class="a-center no-border"> <?php echo $row["dcty_name"]; ?></td>   
												...
												<td class="a-center no-border"> <?php echo $row["dcty_cutrate"]; ?></td>  
											</tr> 
